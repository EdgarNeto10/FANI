...
(function($) {
    $.fn.linker = function(options) {
        ...
        function drawPath(p1, p2) {
            ...
            nn = document.getElementById("linker_paths").appendChild(p);
            var atributo = nn.getAttribute("d");
            listA.push(atributo)

            sessionStorage.setItem("path", JSON.stringify(listA))
        ...
        this.node = function(data) {
            ...
            node.el = $("<div class=\"linker_node node_" + node.id + "\" style=\"left:" + node.x + "px;top: " + node.y + "px;\" id=" + node.id + "><h3>" + node.name + " <span class=\"remove\"></span></h3><div class=\"linker_inputs\"></div><div class=\"linker_outputs\"></div> </div>");
            ...
            node.inputs = [];
            var c = 1
            node.input = function(id, name, id2 = Math.random())

            {
                ...
                var label = $("<div class=\"linker_label\"><span id=" + id2 + ">" + name + "</span></span></div>").append(input.el);
            ...
            node.outputs = [];
            node.output = function(id, name, id2 = Math.random()) {
                ...
                var label = $("<div class=\"linker_label\"><span id=" + id2 + ">" + name + "</span></div>").append(output.el);
            ...
            // Função criada para apanhar node clicado


            $("h3", node.el).click(function() {
                node.onClick();
            })
